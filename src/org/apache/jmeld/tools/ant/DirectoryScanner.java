       ...
       implements SelectorScanner, ResourceFactory
{
    ...
    try
    {
      synchronized (this)
      {
        ...
        for (int i = 0; i < excl.length; i++)
        {
          ...
          {
            scandir(
              new File(basedir, excl[i]),
              excl[i] + File.separator,
              false);
          }
        }
  ...
  protected boolean couldHoldIncluded(String name)
  {
    for (int i = 0; i < includes.length; i++)
    {
      if (matchPatternStart(
          includes[i],
          name,
          isCaseSensitive()) && isMorePowerfulThanExcludes(name, includes[i])
   ...
   *  @since Ant 1.6
   */
  private boolean isMorePowerfulThanExcludes(
    String name,
   ...
   * @since Ant 1.6.3
   */
  private String[] fillNonPatternSet(
    Set      set,
    String[] patterns)
  {
