...
//        setIntercellSpacing(spacing); // for both row margin and column margin
  ...
  private void init(TreeTableCellRenderer renderer)
  {
    ...
    setDefaultRenderer(AbstractTreeTableModel.hierarchicalColumnClass, renderer);
    ...
    PropertyChangeListener l = new PropertyChangeListener()
      {
        ...
        {
          JXTreeTable.this.renderer.putClientProperty(
            evt.getPropertyName(),
            evt.getNewValue());
  ...
  private void initActions()
  {
    ...
    map.put(
      "expand-all",
      new Actions("expand-all"));
    map.put(
      "collapse-all",
      new Actions("collapse-all"));
  }
...
//        return new TreeTableHacker();
  ...
  public class TreeTableHacker
  {
     ...
     */
    public void hitHandleDetectionFromEditCell(
      int         column,
      EventObject e)
    {
     ...
     *         expand/collaps
     */
    protected boolean expandOrCollapseNode(
      int         column,
      EventObject e)
    {
      ...
      {
        MouseEvent pressed = new MouseEvent(renderer,
            me.getID(),
            me.getWhen(),
            me.getModifiers(), me.getX() - getCellRect(0, column, false).x,
            me.getY(),
            me.getClickCount(),
            me.isPopupTrigger());
   ...
   */
  @Override
  public final void setRowHeight(
    int row,
    int rowHeight)
  {
    throw new UnsupportedOperationException(
      "variable height rows not supported");
  }
    ...
    int               column)
  {
    ...
    return applyRenderer(
      component,
      getComponentAdapter(row, column));
  }
   ...
   */
  protected Component applyRenderer(
    Component        component,
   ...
   */
  public TreePath getPathForLocation(
    int x,
    int y)
  {
        ...
        new Runnable()
        {
          public void run()
          {
            ...
            if (indices != null)
            {
              ...
              {  // Dont bother to update if the parent 
                 // node is collapsed
            ...
            {  // case where the event is fired to identify root.
              fireTableDataChanged();
            }
          }
        });
...
//                p.translate(-pathBounds.x, -pathBounds.y);
        MouseEvent newEvent = new MouseEvent(rComponent,
            event.getID(),
            event.getWhen(),
            event.getModifiers(), mousePoint.x, mousePoint.y,
            //                    p.x, p.y, 
          event.getClickCount(),
